...
const AllEventos = (props) => {
    ...
    useEffect(() => {
        const fetchEventos = async () => {
            try {
                setLoading(true);
                const response = await axios.get("http://localhost:4000/pontosTodos", {
                    params: { userId: props.usuarioId }
                });
                const data = response.data;
                console.log("dados de todos os eventos: ", response.data);
    ...
    return (
        <div>
            <h1 className="title is-1">Lista de Eventos para se inscrever: </h1>
            <div className="mb-3">
